...
data Frame = Frame {
    ...
    value_concepts :: [(Concept, Type, (Int, Int))], --concepts with a numerical value as second element
...
data Template = Template { 
    ...
    -- min_body_atoms :: Int,
    -- max_body_atoms :: Int,
    num_causal_rules :: Int,
    num_regular_successions :: Int
    -- max_head_atoms :: Int
} deriving (Eq, Ord, Show)


data GroundAtom =   GA Concept [Object] |
                    ValGA Concept Object Int |
                    Perm Concept [Object] |
                    NGA Concept [Object] Int
                    deriving (Eq, Ord)

data VarAtom =      VA Concept [Var] |
                    ValVA Concept Var Int|
                    Isa Concept Var |
                    Isa2 Concept Var Var
                    deriving (Eq, Ord)

-- ADJUSTED
data Rule = Regsucc RuleID [Atom] Atom |
            Causal RuleID [Atom] Atom |
            -- Arrow RuleID [Atom] Atom | 
...
data InterpretationStatistics = IS {
    num_used_regular_successions :: Int,
    num_used_causal_rules :: Int,
...
do_fig_template add_const t dir input_f space_time = do
    ...
    (script, results) <- gen_bash d input add_const t space_time
...
interpretation_lines t = es ++ ts ++ urs ++ stats where
    es = gen_elements (frame t)
    ts = gen_typing t
    -- (crs, n) = gen_conceptual_rules t
    (urs, n1) = gen_update_rules t
    -- cjs = gen_causal_judgements t n1
...
gen_update_rules :: Template -> ([String], Int)
gen_update_rules t = (h ++ cs ++ reg_succs ++ causes ++ uses ++ [""],  n1) where
    ...
    reg_succs = [ fr i | i <- [1 .. 1 + num_regular_successions t - 1]]
    n = 1 + num_regular_successions t
    causes = [ fc i | i <- [n .. n + num_causal_rules t - 1]]
    n1 = n + num_causal_rules t
    fr i = "is_regsucc_rule(r" ++ show i ++ ")."
    fc i = "is_causal_rule(r" ++ show i ++ ")."
    uses = ["{ use_rule(R) } :- is_regsucc_rule(R).", "{ use_rule(R) } :- is_causal_rule(R)."]
...
gen_inits name t = do
    ...
    appendFile f "\n"
    print_max_1_constraint f as
    putStrLn $ "Generated " ++ f


print_max_1_constraint :: String -> [GroundAtom] -> IO()
print_max_1_constraint f as = Monad.forM_ cons_no_dups (print_constraint f) where
    cons_no_dups = remove_dups valGAs_list
    valGAs_list = filter(\a -> is_val_atom a) as


print_constraint :: String -> GroundAtom -> IO ()
print_constraint f (ValGA c o _) =
    appendFile f constraint where
        constraint = ":- init(" ++ show c ++ ", " ++ show o ++ ", X), init(" ++ show c ++ ", " ++ show o ++ ", Y), X != Y.\n"

print_constraint f _ = return ()


remove_dups :: [GroundAtom] -> [GroundAtom]
remove_dups [] = []

remove_dups (x : xs) = case is_val_atom x of
    True -> result where
        result_rest = remove_dups(xs)
        result = if check_in_result x result_rest then result_rest
                else (x : result_rest)

    False -> remove_dups(xs)


check_in_result :: GroundAtom -> [GroundAtom] -> Bool
check_in_result x xs = or list_of_bools where
    list_of_bools = map f xs
    f y = check_same_conc_and_obj x y 

check_same_conc_and_obj :: GroundAtom -> GroundAtom -> Bool
check_same_conc_and_obj (ValGA c1 o1 _) (ValGA c2 o2 _) = (c1,o1) == (c2,o2)

check_same_conc_and_obj _ _ = False


is_val_atom :: GroundAtom -> Bool
is_val_atom (ValGA _ _ _) = True
is_val_atom _     = False
...
is_static_atom t (ValGA c _ _) = c `elem` (static_concepts (frame t))
...
gen_bash :: String -> String -> Bool -> Template -> String -> IO (String, String)
gen_bash dir input_f add_const t space_time = do
    ...
    let args = "--warn=no-atom-undefined --quiet=1 --time-limit=" ++ show const_time_limit ++ " "
    let args' = if add_const then args ++ "-c k_xor_group=$1 " ++ xor_group_file_name ++ " " else args
    let clingo = if flag_condor then "/vol/lab/clidngo5/clingo " else "clingo "
    -- let costs = if flag_ablation_remove_cost then " " else " asp/costs.lp "
    let back = "asp/figurative_backbone.lp "
    let cons = "asp/figurative_constraints.lp "
    let events = "asp/figurative_events.lp "
    let judg = "asp/figurative_judgements.lp "
    let synth = "asp/figurative_synthesis.lp "
    let temp = "asp/temporal_axioms.lp "
    let fig_files = back ++ cons ++ events ++ judg ++ synth ++ temp
    let s_files = case space_time of       
            "time" -> " "
            "space" -> space ++ spacesynth where
                            space = "asp/spatial_axioms.lp "
                            spacesynth = "asp/spatial_synthesis.lp " 
    let s = clingo ++ args' ++ task_file ++ " " ++ init_f ++ " " ++ subs_f ++ " " ++ rules_f ++ " " ++ interpretation_f ++ " " ++ fig_files ++ s_files ++ handle ++ "\n\n"
    appendFile f s
    putStrLn $ "Generated " ++ f
...
var_subgroups :: Frame -> [Vars]
var_subgroups frm = List.nub $ list_of_subgroups where
    list_of_subgroups = concat list_of_subgroupslists
    list_of_subgroupslists = map List.subsequences (var_groups frm)
...
gen_var_atoms name t = do
    ...
    Monad.forM_ (all_var_value_atoms frm) (print_var_fluent f frm)
...
all_var_groups t (VA _ vs) = filter f (var_groups t) where
    f vs2 = vs `subset` vs2
all_var_groups t (ValVA _ v _) = filter f (var_groups t) where
    f vs2 = v `elem` vs2    
...
get_value_concepts :: Template -> [(Concept, Type, (Int,Int))]
get_value_concepts = value_concepts . frame
...
-- check :: [(Concept, [Type])] -> IO ()
-- check cs = case cs of
--         [] -> putStrLn $ "Lege concepts"
--         _ -> putStrLn $ "Volle concepts"

all_ground_atoms :: Template -> [GroundAtom]
all_ground_atoms template = res where 
    res = ss1 ++ ss2 ++ isas
    ss1 = concat (map f cs)
    ss2 = concat (map f2 vs)    
    cs = get_fluid_concepts template
    vs = get_value_concepts template
...
-- Maps all 'object_tuples' from types in ts to GA concept tuple
    ...
    f2 (c, typ, (i1, i2)) = map (\(obj, int) -> ValGA c obj int) (all_obj_value_pairs template typ (i1,i2))
...
all_obj_value_pairs :: Template -> Type -> (Int, Int) -> [(Object,Int)]
all_obj_value_pairs template t (i1, i2) = res where

    res = [(obj, val) | obj <- objects, val <- values]
    objects = map fst (filter f (get_objects template))
    f (_, t') = (t', t) `elem` sts
    sts = sub_types_star (frame template)

    values = ints_in_range(i1,i2)

ints_in_range :: (Int, Int) -> [Int]
ints_in_range (i1, i2) = if i1 > i2 then []
                         else if i1 == i2 then [i1]
                         else (i1 : xs) where
                            xs = ints_in_range(i1+1, i2)


-- ints_in_range :: (Int, Int) -> [Int]
-- ints_in_range (i1, i1) = [i1]

-- ints_in_range :: (Int, Int) -> [Int]
-- ints_in_range (i1, i2) = (i1 : xs) where
--     xs = ints_in_range(i1+1, i2)
...
-- DIT ALLEMAAL NOG CHECKEN
all_var_value_atoms :: Frame -> [VarAtom]
all_var_value_atoms frm = concat (map f (value_concepts frm)) where
    f (c, one_type, (i1, i2)) = map (\(var, int) -> ValVA c var int) (all_subtype_value_pairs frm one_type (i1,i2))


    -- f2 (c, typ, (i1, i2)) = map (\(obj, int) -> ValGA c obj int) (all_obj_value_pairs template typ (i1,i2))


all_subtype_value_pairs :: Frame -> Type -> (Int, Int) -> [(Var, Int)]
all_subtype_value_pairs frm typ (i1, i2) = res where
    res = [(var, i3) | var <- vs, i3 <- values]
    -- objects = map fst (filter f (get_objects template))

    values = ints_in_range(i1,i2)

    -- frm = frame template
    -- subtypes = map fst (filter f (sub_types_star (frame template))) 

    vs = map fst (filter f (vars frm))
    f (_, t') = (t', typ) `elem` sub_types_star frm
...
my_divide_into_3groups2 :: Ord a => [a] -> [([a],[a],[a])] -> [([a],[a],[a])]
my_divide_into_3groups2 [] acc = acc
my_divide_into_3groups2 (x:xs) acc = acc1 ++ acc2 ++ acc3 where
...
insert_second :: Ord a => a -> [([a],[a],[a])] -> [([a],[a],[a])]
insert_second x acc = map f acc where
    f (a, b, c) = (a, (x:b), c)

insert_third :: Ord a => a -> [([a],[a],[a])] -> [([a],[a],[a])]
insert_third x acc = map f acc where
    f (a, b, c) = (a, b, (x:c))


-- divide_into_groups [1,2,3] = [[[1,2,3]],[[1,2],[3]],[[1],[2,3]],[[1],[2],[3]]]
...
-- Als a 'answer' als prefix heeft wordt b toegevoegd..
...
readable_interpretation t i = is ++ ps ++ rs ++ xs ++ ss ++ acs where
    is = case inits i of
        [] -> []
...
readable_stats i = [
    ...
    "----------",
    "",    
    "Num regular successions: " ++ show (num_used_regular_successions i),
    "Num causal rules: " ++ show (num_used_causal_rules i),
    ...
    "Num used causal judgements: " ++ show(num_used_causal_judgements i)
...
latex_stats_table i = 
    show (num_used_regular_successions i) ++ " & " ++ 
    show (num_used_causal_rules i) ++ " & " ++ 
...
total_cost i = 
    num_used_regular_successions i + 
    num_used_causal_rules i + 
    total_body_atoms i +
    num_inits i
...
extract_statistics xs = IS {
    num_used_regular_successions = extract_num_used_regular_successions xs,
    num_used_causal_rules = extract_num_used_causal_rules xs,
    ...
    num_used_causal_judgements = extract_num_used_causal_judgements xs
    } 
...
extract_num_used_regular_successions xs = length (extract_atoms "used_regular_succession(" xs)
extract_num_used_causal_rules xs = length (extract_atoms "used_causal_rule(" xs)
...
extract_num_used_causal_judgements xs = length(extract_atoms "used_causal_judgement(" xs)
...
extract_rules xs = extract_regular_successions xs ++ extract_causal_rules xs


extract_regular_successions :: [String] -> [Rule]
extract_regular_successions xs = map f (extract_regsucc_heads xs) where
    f (r, c) = Regsucc r (extract_body xs r) c


extract_causal_rules :: [String] -> [Rule]
extract_causal_rules xs = map f (extract_causal_heads xs) where
    f (r, c) = Causal r (extract_body xs r) c



extract_regsucc_heads :: [String] -> [(RuleID, Atom)]
extract_regsucc_heads xs = Maybe.mapMaybe f xs where
    p = "regsucc_head_co("
    f x = case List.isPrefixOf p x of
        False -> Nothing
        True -> Just $ 
...
extract_causal_heads :: [String] -> [(RuleID, Atom)]
extract_causal_heads xs = Maybe.mapMaybe f xs where
    p = "causal_head_co("
    f x = case List.isPrefixOf p x of
        False -> Nothing
        True -> Just $ 
...
regsucc_string :: String 
regsucc_string = if flag_unicode then " → " else " -> "


arrow_string :: String 
arrow_string = if flag_unicode then " → " else " -> "

causal_string :: String
causal_string = if flag_unicode then " ▸ " else " >> "
...
instance Show Rule where
    show (Regsucc r bs h) = r ++ " : " ++ concat (List.intersperse and_string bs) ++ regsucc_string ++ h
    show (Causal r bs c) = r ++ " : " ++ concat (List.intersperse and_string bs) ++ causal_string ++ c
...
latex_rule (Regsucc r bs h) = t where
    t = bst ++ " \\rightarrow " ++ ht ++ "\\\\"
    bst = concat (List.intersperse " \\wedge " bs2)
    bs2 = map convert_atom bs
    ht = convert_atom h
latex_rule (Causal r bs h2) = t where
    t = bst ++ " \\fork " ++ ht2 ++ "\\\\"
    bst = concat (List.intersperse " \\wedge " bs2)
    bs2 = map convert_atom bs
    ht2 = convert_atom h2
...
instance Show GroundAtom where
    ...
    show (ValGA c o i) = "init(s2v(" ++ show c ++ ", " ++ show o ++ ", " ++ show i ++ "))"
...
instance Show VarAtom where
    ...
    show (ValVA c x i) = "s2v(" ++ show c ++ ", " ++ show x ++ ", " ++ show i ++ ")"
...
-- Dus volgens deze bias kan je niet holds(s2(X,1)) en holds(s2(X,2)) hebben
...
-- modeh geeft een mode declaration voor heads...
-- alleen snap ik niet wat hier precies staat... in XHAIL moet er '+,-,$' voor een argument
-- en waarom twee keer hetzelfde argument?
